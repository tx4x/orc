...
class RenterProfile extends Profile {
   ...
   */
  _init() {
    this.capacity = tiny(this.config.RenterCapacityCachePath);
    ...
    let opts = {
      ...
      enableSSL: parseInt(this.config.BridgeUseSSL),
      serviceKeyPath: this.config.BridgeServiceKeyPath,
      certificatePath: this.config.BridgeCertificatePath,
      authorityChains: this.config.BridgeAuthorityChains
    };
    ...
    this.bridge = new Bridge(this.node, opts);
...
class DirectoryProfile extends Profile {
   ...
   */
  _init() {
    this.capacity = tiny(this.config.DirectoryCapacityCachePath);
    ...
    let opts = {
      capacityCache: this.capacity,
      enableSSL: !!parseInt(this.config.DirectoryUseSSL),
      serviceKeyPath: this.config.DirectoryServiceKeyPath,
      certificatePath: this.config.DirectoryCertificatePath,
      authorityChains: this.config.DirectoryAuthorityChains
    };

    this.directory = new Directory(this.node, opts);

    this.node.logger.info(
      'establishing public directory server at ' +
      `${this.config.DirectoryHostname}:${this.config.DirectoryPort}`
    );
    this.directory.listen(parseInt(this.config.DirectoryPort),
                          this.config.DirectoryHostname);
  }
