...
class Bridge extends EventEmitter {
  ...
  _bindRoutes() {
    ...
    this.api.use(serveStatic(path.join(__dirname, '../web')));

    // NB: Generate stats endpoint
    this.api.get('/statuses', this.getNodeStatus.bind(this));
  ...
  getNodeStatus(req, res, next) {
    this.node.shards.size((err, capacity) => {
      if (err) {
        return next(err)
      }
