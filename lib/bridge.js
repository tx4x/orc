...
class Bridge extends EventEmitter {
  ...
  distribute(filepath, object, callback) {
    ...
    const uploadShards = (shards, object, callback) => {
      ...
      // TODO: choose top 0.75 for shards
      // TODO: split parity between .75 - .25 and .25 - 0
      // TODO: pool size = shards.length || parity.length

      async.eachLimit(shards, 3, (shard, next) => {
        async.retry({ times: 10 }, (done) => {
          this._pluckStorageProvider(shard.size, [0, 1], 1, (err, target) => {
            if (err) {
              return done(err);
            }
  ...
  _rebuildObject(object, callback = () => null) {
    ...
    this._downloadObject(object.id, (err, buffer, object, info) => {
      ...
      // TODO: if failed length <= parity, choose bottom 0.5
      // TODO: else choose bottom 0.75 for parity, top .75 for others
      // TODO: pool = failed length
