...
class Bridge extends EventEmitter {
  ...
  constructor(node, options) {
    ...
    this.wss = this.options.enableControlProxy ? new ws.Server({
      server: this.server,
  ...
  listen() {
    ...
    /* istanbul ignore if */
    if (!this.options.enableControlProxy) {
      return;
    }
  ...
  upload(req, res, next) {
    ...
    busboy.once('file', (field, file, name, encoding, mime) => {
      ...
      file.pipe(hasher).pipe(cipher).pipe(writer).on('finish', () => {
        ...
        try {
          let old = tmp;
          tmp = path.join(path.dirname(tmp), digest);

          fs.renameSync(old, tmp);
        } catch (err) {
          /* istanbul ignore next */
          return next(new Error('Failed to rename object staging directory'));
        }
