...
class Bridge {
    ...
    this.database.ObjectPointer.find(query, (err, objects) => {
      ...
      async.eachSeries(
        ...
        (err) => {
          ...
          this._publishAuditReports(callback);
        }
      );
    });
  }
  ...
  _auditObject(object, callback) {
    async.mapLimit(object.shards, 3, (shard, done) => {
      ...
      async.eachLimit(results, ([shard, contract], done) => {
        ...
        if (shard.challenges.length === 0) {
          this._regenerateChallenges(shard);
        }
        ...
        this.node.auditRemoteShards(contact, audits, (err, proofs) => {
          const fail = (err, expected, actual) => {
            ...
            this.node.logger.warn(err.message);
            this.database.AuditReport.create({
          ...
          if (!proof) {
            return fail(new Error('Shard failed audit with missing proof'),
                        '000000', '000000');
          }
  ...
  _rebuildObject(object, callback = () => null) {
    ...
    // TODO: Upload shards that were marked as decayed to new hosts
    // TODO: Update shard references in pointer
    // TODO: Nullify decayed contracts
  ...
  _regenerateChallenges(shard, callback = () => null) {
    // TODO: Download the individual shard
    // TODO: Generate new challenges via AuditStream
    // TODO: Update the shard's contract
    // TODO: Sign renewal contract
    // TODO: Renew with host
    // TODO: Save updated response contract
    callback(new Error('Not implemented'));
  }
   ...
   * @private
   */
  _publishAuditReports(callback = () => null) {
    // TODO: Load all audit reports
    // TODO: Sign each one
    // TODO: Consolidate into IMP-11 payload format
    // TODO: Send to directory bootstrap
    // TODO: Send to local directory
    // TODO: If successful, remove all reports
    // TODO: If fails, leave for next interval
    callback(new Error('Not implemented'));
  }
