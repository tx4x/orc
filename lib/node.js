...
class Node extends kad.KademliaNode {
  ...
  _bootstrap() {
    ...
    this.database.PeerProfile.findOneAndUpdate(
      { identity: this.identity.toString('hex') },
      { contact: this.contact, updated: Date.now() },
      { upsert: true }
    );
  }

  /**
   * Updates the local peer capacity profile
   * @param {object[]} capacityData
   */
  updateProviderCapacity([capacity, contact]) {
    return new Promise((resolve, reject) => {
      let timestamp = Date.now();

      this.database.PeerProfile.findOneAndUpdate({ identity: contact[0] }, {
        capacity: {
  ...
  /**
   * Triggers a capacity annoucment
   */
  announceCapacity() {
    return new Promise((resolve, reject) => {
      this.shards.size((err, data) => {
        ...
        this.publishCapacityAnnouncement(data, (err) => {
          /* istanbul ignore if */
          if (err) {
            this.logger.error(err.message);
            this.logger.warn('failed to publish capacity announcement');
          } else {
            this.logger.info('published capacity announcement ' +
