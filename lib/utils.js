 ...
 * @returns {object}
 */
module.exports.bufferAsReadableStream = function(buffer) {
  return new stream.Readable({
    read: function() {
      if (this._bufferDrained) {
        this.push(null);
      } else {
        this._bufferDrained = true;
        this.push(buffer);
      }
    }
  });
 ...
 * @returns {object}
 */
module.exports.getCapacityFromFlags = function(flags = []) {
  let capacity = {
    allocated: 0,
    available: 0
  };
  ...
  try {
    for (let [flag, value] of flags) {
      switch (flag) {
        case 'ALLOCATED':
          capacity.allocated = value;
          break;
        case 'AVAILABLE':
