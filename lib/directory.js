...
class Directory {
  ...
  getPeerReputationScore(identity, callback) {
    ...
    async.series([
      ...
      (next) => {
        async.parallel([
          (done) => this.database.PeerProfile.count({
            'reputation.score': { $gt: target.reputation.score }
          }, (err, count) => {
            peers.upper = count;
            done(err);
          }),
      ...
      (next) => {
        percentile = target.reputation.score / highest.reputation.score;
    ...
    ], (err) => callback(err, { percentile, allowance, score, capacity }));
  }
  /**
  ...
  _getTotalKnownNetworkCapacity(callback) {
    ...
    // TODO: Aggregate total known available and allocated capacity

    callback(null, { available, allocated });
  }
