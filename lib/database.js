...
ObjectPointer.virtual('percentDecayed').get(function() {
  ...
  const auditDecay = this._lastAuditTimestamp < decayTime;
  const accessDecay = this._lastAccessTimestamp < decayTime;

  if (auditDecay && accessDecay) {
    return 1;
  }
