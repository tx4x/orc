...
class Rules {
  ...
  audit(request, response, next) {
    ...
    // TODO Add audit result record and report to directory
  ...
  renew(request, response, next) {
    ...
    if (!(!renewal.validateSync() && renewal.isComplete)) {
  ...
  claim(request, response, next) {
    ...
    const xpub = contract.ownerParentKey;
    ...
    if (!(!contract.validateSync() && contract.isComplete)) {
